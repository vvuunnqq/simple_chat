defmodule Chatter.Account.UserToken do
  use Ecto.Schema
  import Ecto.Query
  ...
  schema "users_tokens" do
    ...
    belongs_to :user, Chatter.Account.User
  ...
  def build_session_token(user) do
    ...
    {token, %Chatter.Account.UserToken{token: token, context: "session", user_id: user.id}}
  end
  ...
  def token_and_context_query(token, context) do
    from Chatter.Account.UserToken, where: [token: ^token, context: ^context]
  end
  ...
  def user_and_contexts_query(user, :all) do
    from t in Chatter.Account.UserToken, where: t.user_id == ^user.id
  end

  def user_and_contexts_query(user, [_ | _] = contexts) do
    from t in Chatter.Account.UserToken, where: t.user_id == ^user.id and t.context in ^contexts
  end
end
